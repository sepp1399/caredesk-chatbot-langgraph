"""Lead-creation agent — logs a callback request for the call center."""
...
from app.tools.lead_creation import LEAD_CREATION_TOOLS
from app.tools.shared.transfer import transfer_to_flow
from app.i18n import apply_language, system_prompt_suffix

logger = logging.getLogger("caredesk_lg.lead_creation")

_PROMPT_PATH = Path(__file__).parent / "lead_creation_prompt.md"
...
lead_creation_agent = create_react_agent(
    model=llm,
    tools=[*LEAD_CREATION_TOOLS, transfer_to_flow],
    pre_model_hook=_pre_model_hook,
    checkpointer=_memory,
)
logger.info("LeadCreation agent ready")
